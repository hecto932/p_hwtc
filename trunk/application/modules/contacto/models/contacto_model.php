...
class Contacto_model extends CI_Model {
	...
	function guardar_testimonio($data)
	{
		$data['id_estado'] = 2;
		$data['rating'] = 3;
	...
	function get_testimonios()
	{
		...
		$this->db->order_by('t.id_testimonio DESC');
		$this->db->limit(10);
		return $this->db->get('testimonio t')->result();
	}
	
	function existe_testimonio($comentario)
	{
		...
		SELECT * FROM
		(
			SELECT t.id_testimonio, t.comentario
			FROM testimonio t
			ORDER BY t.creado DESC
			LIMIT 100
		) te
		WHERE LOWER(te.comentario) = ".$this->db->escape($comentario)."
		";
