...
class Banner extends MX_Controller {
	...
	function imagen_procesar()
	{
		...
		foreach($imagenes as $imagen)
		{
			...
			//Multimedia carpetas
			$id_imagen = $this->multimedia_model->guardar_imagen($data_img, 1600, 700, 600, 239, 300, 88);
			
			//OJO Si el banner ya tiene multimedia asociada
			$tiene_imagen = $this->existe_imagen($imagen['id']);
			if($tiene_imagen) $this->multimedia_model->delete_image($id_imagen, 'banner', $imagen['fichero']);
			
			//Relacion multimedia - banner
			$data_rel = array(
								'id_banner' => $imagen['id'],
	...
	function existe_imagen($id_banner)
	{
		$this->load->model('multimedia/multimedia_model');
		$this->multimedia_model->existe_imagen_banner($id_banner);
	}
