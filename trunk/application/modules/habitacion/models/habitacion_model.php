...
class habitacion_model extends CI_Model {
	...
	{
		$this->db->select('detalle_habitacion.*, habitacion.*');
	...
	*/
	...
	function get_all($idioma = 'es'){
		...
		foreach($q as $row => $val)
		{
			...
			if(isset($q[$row_plus]))
			{
				if(($q[$row]->id_habitacion == $q[$row_plus]->id_habitacion))
				{
					unset($q[$row_plus]);
				}
			}
	...
	function get_page($start = 0, $count = 10, $order_field='habitacion.id_habitacion', $order_dir = 'desc', $terminos_busqueda = array()){
		...
		$this->db->select('habitacion.*,detalle_habitacion.*, habitacion.id_habitacion as id_habitacion, multimedia.fichero');
		$this->db->join('detalle_habitacion','habitacion.id_habitacion=detalle_habitacion.id_habitacion','left');
	...
	*/
	...
	function get_habitaciones()
	{
		$query =
		"
			SELECT hh.*, h.*, GROUP_CONCAT(mu.fichero SEPARATOR ', ') as fichero
			FROM habitacion h
			JOIN detalle_habitacion hh ON hh.id_habitacion = h.id_habitacion
			
			LEFT JOIN (	SELECT m.*, r.id_habitacion
						FROM rel_habitacion_multimedia r
						JOIN multimedia m ON m.id_multimedia = r.id_multimedia
						WHERE m.destacado = 2) mu ON mu.id_habitacion = h.id_habitacion
			
			WHERE h.id_estado = 1
			GROUP BY h.id_habitacion
			ORDER BY h.destacado ASC, h.id_habitacion ASC
		";
		return $this->db->query($query)->result();
		
	}
	
	function get_habitacion_url($url)
	{
		$query =
		"
			SELECT rr.*, r.*, group_concat(muu.fichero SEPARATOR ', ') as fichero
			FROM habitacion r
			JOIN detalle_habitacion rr ON rr.id_habitacion = r.id_habitacion
			
			LEFT JOIN (	SELECT mm.*, rr.id_habitacion
						FROM rel_habitacion_multimedia rr
						JOIN multimedia mm ON mm.id_multimedia = rr.id_multimedia
						WHERE mm.destacado = 2 AND mm.fichero IS NOT NULL) muu ON muu.id_habitacion = r.id_habitacion
