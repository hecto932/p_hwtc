...
class Habitacion extends MX_Controller
{
    ...
    function listado($order_field = 'habitacion.id_habitacion', $order_dir = 'desc', $start = 0, $ajax = false)
    {
        ...
        {
            $terminos_busqueda['habitacion.destacado'] = $_POST['destacado'];
        }
	...
	function _validar_url($url)
	{
		...
		$datos = modules::run('services/relations/tabla_url', 'habitacion', $url);
		$id_detalle_habitacion = ($this->input->post('id_detalle_habitacion')) ? $this->input->post('id_detalle_habitacion') : '';
		
		if(!empty($datos))
		{
			if(!empty($id_detalle_habitacion) && $id_detalle_habitacion == $datos[0]->id_detalle_habitacion && $datos[0]->id_idioma == $this->input->post('id_idioma'))
			{
				return TRUE;
			}
			else return FALSE;
    ...
    function eliminar_idioma($id_habitacion, $id_detalle_habitacion = '', $ajax = false)
    {
        ...
        if ($ajax)
            ...
            redirect('backend/habitaciones/ficha_habitacion/' . $id_habitacion);
    }
