<div class="container_16 descripcion">
	...
	<h2 style="color:#757575; margin-left:35px;">
		...
		<span style="color:#00a9dd; font-weight:normal;"> > 
			<?php if(isset($padre_nombre)&&($padre_nombre!='')){ echo $padre_nombre; } ?>
		</span>
	</h2>
	<hr id="hrfooter" />
		...
		<?php 	foreach ($subcategorias as $categoria)
				{
					...
					if(isset($categoria->url) && ($categoria->url!=''))
					{ $url=$categoria->url; }else{ $url=$categoria->id_categoria; }
				...
				<li>
					<span><h2><?php if(strlen($nombre)>26){ echo substr($nombre,0,20).'...'; }else{ echo $nombre; } ?></h2></span>
					<img width="200" src="<?php echo $img_url; ?>" />
					<p><?php echo $categoria->descripcion_breve.'<br>'; ?></p>
					<a href="<?php echo site_url().lang('listado_productos_url').$url; ?>"><?php echo lang('productos_leer'); ?></a>
				</li>
	   <?php } ?>
	</ul>
	<?php } ?>
