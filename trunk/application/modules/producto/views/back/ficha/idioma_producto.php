...
<script type="text/javascript">
	...
	$(document).ready(function()
	{
		...
		{
		    var i = $('#p_caracts .p_countInput').length;
		    
		    $('#p_caracts').append
		    (
		    	'<div id="p_caract_'+i+'" class="twelve columns" >'+
		    		'<div class="ten mobile-three columns">'+
						'<input name="caract[]" type="text" class="p_countInput" />'+
			  		'</div>'+
			  		'<div class="one mobile-one columns">'+
			  			'<a class="button expand postfix" onclick="p_agregarsub(this)"> + </a>'+
			  		'</div>'+
			  		'<div class="one mobile-one columns">'+
			  			'<a class="button expand postfix" onclick="p_eliminar(this)" > - </a>'+
			  		'</div>'+
			  	'</div>'
		    );
	...
	function p_agregarsub(addsub)
	{
		...
		$('#'+idCarat).append
		(
			'<div id="p_subcaract_'+i+'" >'+
	    		'<div class="nine mobile-three columns">'+
					'<input name="caract[][]" type="text" class="p_countsubInput" />'+
		  		'</div>'+
...
<?php echo form_open(lang('backend_url').'/'.lang('productos_url').'/'.lang('guardar_url').'_'.lang('idioma_url'),'id="gen_form" class="custom"'); ?>
<br />
<?php /*--------------------------------- NOMBRE ---------------------------------*/ ?>
	<div class="six columns">
		...
		<?php $temp = (isset($producto->nombre)) ? $producto->nombre : ''; ?>
			...
			<select class="custom lenguaje_productos"  name="id_idioma">
				<?php foreach($idioma_select as $im): ?>
					...
					elseif ($accion != 'normal')
						echo ((isset($producto->id_idioma) && $producto->id_idioma==$im->id_idioma) ? ' selected="selected"' : '')?>><?php echo ucfirst($im->nombre); ?></option>
				<?php endforeach; ?>
			</select>
	</div>
...
<?php /*--------------------------------- TITULO_PAGINA ---------------------------------*/ ?>
	<div class="six columns">
        ...
        <?php $temp = (isset($producto->titulo_pagina)) ? $producto->titulo_pagina : ''; ?>
...
<?php /*--------------------------------- KEYWORDS ---------------------------------*/ ?>
	<div class="six columns">
		...
		<?php $temp = (isset($producto->keywords)) ? $producto->keywords : ''; ?>
...
<?php /*--------------------------------- DESCRIPCION_PAGINA ---------------------------------*/ ?>
	<div class="twelve columns">
		...
		<?php $temp = (isset($producto->descripcion_pagina)) ? $producto->descripcion_pagina : ''; ?>
...
<?php /*--------------------------------- PRESENTACION ---------------------------------*/ ?>
	<div class="twelve columns">
		...
		<?php $temp = (isset($producto->presentacion)) ? $producto->presentacion : ''; ?>
...
<?php /*--------------------------------- URL ---------------------------------*/ ?>
	<div class="twelve columns">
		...
		<?php $temp = (isset($producto->url)) ? $producto->url : ''; ?>
...
<?php /*--------------------------------- DESCRIPCION_BREVE ---------------------------------*/ ?>
	<div class="twelve columns">
        ...
        <?php $temp = (isset($producto->descripcion_breve)) ? $producto->descripcion_breve : ''; ?>
		<textarea class='<?php echo (form_error("descripcion_breve") != "") ? "error" : "" ; ?>' name="descripcion_breve" rows="3" cols="50"><?php echo idioma_values(set_value('descripcion_breve'), $temp, $accion); ?></textarea>
...
<?php /*--------------------------------- DESCRIPCION_AMPLIADA1 ---------------------------------*/ ?>
	...
	<div class="twelve columns">
		...
		<?php endif; ?>
		<?php $temp = (isset($producto->descripcion_ampliada)) ? $producto->descripcion_ampliada : ''; ?>
...
<?php /*--------------------------------- CARACTERISTICAS ---------------------------------*/ ?>
	...
	<div class="twelve columns" style="padding-bottom: 20px; padding-top: 20px;">
		...
		<input type="button" class="one mobile-four column small button" id="p_plus" value="Agregar" />
	...
	<div id="p_caracts">
		...
		<?php $caracteristicas = (isset($caracteristicas)) ? $caracteristicas : json_decode($producto->caracteristicas); ?>
		<?php if(isset($caracteristicas) && !empty($caracteristicas)): ?>
		<?php foreach($caracteristicas as $key => $value): ?>

			<!-- $value es un vector que en la posicion 0 tiene la caracteristica -->
			<div id="p_caract_<?php echo $key?>" class="twelve columns">
			
				<!-- Caracteristica -->
				<div class="ten mobile-three columns">
					<input class="p_countInput" type="text" name="caract[]" value="<?php echo $value[0]; ?>">
				</div>
				
				<!-- Botones de caracteristica -->
				<div class="one mobile-one columns"><a class="button expand postfix" onclick="p_agregarsub(this)"> + </a></div>
				<div class="one mobile-one columns"><a class="button expand postfix" onclick="p_eliminar(this)"> - </a></div>
				...
				<?php foreach($value as $subkey => $subcaract): ?>
					...
					<div id="p_subcaract_<?php echo $subkey; ?>" >
						
						<!-- Sub-caracteristica -->
						<div class="nine mobile-three columns">
							<input class="p_countsubInput" type="text" name="caract[][]" value="<?php echo $subcaract; ?>" >
						</div>
						
						<!-- Boton Sub-caracteristica -->
						<div class="one mobile-one columns pull-two"><a class="button expand postfix" onclick="p_eliminar(this)"> - </a></div>
	...
	*/ ?>
	...
	<input type="hidden" name="id_producto" value="<?php echo (isset($id_producto)) ? $id_producto : $producto->id_producto; ?>" />
	<?php if (isset($producto->id_detalle_producto) || $accion != 'normal'): ?>

		<input type="hidden" name="id_detalle_producto" value="<?php echo $producto->id_detalle_producto ?>" />
       	<?php //echo '<input type="hidden" name="id_idioma" value="'.$producto->id_idioma.'" />'; ?>
