...
class servicio_model extends CI_Model {
	...
	function read($id,$id_detalle_servicio='',$idioma='')
	{
		...
		$this->db->select('servicio.*, detalle_servicio.*, servicio.id_servicio as id_servicio');
	...
	function get_all($idioma = 'es'){
		...
		foreach($q as $row => $val)
		{
			...
			if(isset($q[$row_plus]))
			{
				...
				if(($q[$row]->id_servicio == $q[$row_plus]->id_servicio))
				{
					unset($q[$row_plus]);
				}
			}
	...
	*/
	...
	function get_page($start = 0, $count = 10, $order_field='servicio.id_servicio', $order_dir = 'desc', $terminos_busqueda = array()){
		...
		$this->db->select('servicio.*,detalle_servicio.*, servicio.id_servicio as id_servicio, multimedia.fichero');
		$this->db->join('detalle_servicio','servicio.id_servicio=detalle_servicio.id_servicio','left');
	...
	{
		$this->db->select("ss.*, s.*, group_concat(m.fichero SEPARATOR ', ') as fichero", FALSE);
		$this->db->join('detalle_servicio ss', 'ss.id_servicio = s.id_servicio');
		$this->db->join('rel_servicio_multimedia r', 'r.id_servicio = s.id_servicio', 'LEFT');
		$this->db->join('multimedia m', 'm.id_multimedia = r.id_multimedia', 'LEFT');
		$this->db->where('s.id_estado', 1);
		$this->db->group_by('s.id_servicio');
		$this->db->order_by('s.destacado ASC');
		return $this->db->get('servicio s')->result();
	}
