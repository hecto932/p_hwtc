...
class Servicio extends MX_Controller {
    ...
    function listado($order_field = 'servicio.id_servicio', $order_dir = 'desc', $start = 0, $ajax = false) {
		...
		if (isset($_POST['destacado'])) {
            $terminos_busqueda['servicio.destacado'] = $_POST['destacado'];
        }
    ...
    function editar_idioma($id_servicio, $id_detalle_servicio = '')
    {
		...
		$data['servicio'] = $this->servicio_model->read($id_servicio);
		$data['active'] = 'servicios';
		$data['sub'] = 'ficha';
		$data['accion'] = 'editar';
	...
	function _validar_url($url)
	{
		...
		$datos = modules::run('services/relations/tabla_url', 'servicio', $url);
		$id_detalle_servicio = ($this->input->post('id_detalle_servicio')) ? $this->input->post('id_detalle_servicio') : '';
		
		if(!empty($datos))
		{
			if(!empty($id_detalle_servicio) && $id_detalle_servicio == $datos[0]->id_detalle_servicio && $datos[0]->id_idioma == $this->input->post('id_idioma'))
			{
				return TRUE;
			}
			else return FALSE;
