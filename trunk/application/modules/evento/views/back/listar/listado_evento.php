				...
				<table class="twelve" border="1" summary="Tabla de eventos.">
					<thead>
						<tr>
							...
							<!--
							<th id="id_categoria" class="col8<?php echo ((strpos(uri_string(),'id_categoria')!=false) ? strtolower($order_dir) : 'desc')?>"><?php echo anchor((strpos(uri_string(),'id_categoria')!=false) ? $url.'/'.$order_by_new : $url.'/'.'id_categoria/asc',lang("categoria_evento"))?></th>
							-->
					...
					foreach ($eventos as $evento){ ?>
						<tr<?php echo ($i%2==0 ? ' class="dark"' : '') ?> id="evento_<?php echo $evento->id_evento?>">
							...
							<!--
							<td class="col8" headers="categoria">
								<p><?php echo $evento->nombre_categoria; ?></p>
							</td>
							-->
