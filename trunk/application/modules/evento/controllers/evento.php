...
class Evento extends MX_Controller {
    ...
    function listado($order_field = 'evento.id_evento', $order_dir = 'desc', $start = 0, $ajax = false)
    {
        ...
        {
            $terminos_busqueda['evento.destacado'] = $_POST['destacado'];
        }
    ...
    function crear()
    {
		...
		$data['categorias'] 		= $this->evento_model->get_categorias(TRUE);
    ...
    function create($id = '')
    {
        ...
        $this->form_validation->set_rules('fecha_evento', 'Fecha', 'required');
        ...
        else
        {
			...
			*/
			...
			$form_data['fecha_evento'] 	= fecha_to_timestamp($form_data['fecha_evento']);
			$form_data['creado'] 		= date('Y-m-d H:i:s');
            $id = $this->evento_model->update($form_data);
			
			//Guardar en la base de datos respeto al pego del evento
			/*
    ...
    function guardar_idioma()
    {
        ...
        $this->form_validation->set_rules('url', 					lang('evento_idi_url'),		'required|callback__validar_url');
        ...
        if ($this->form_validation->run($this) == FALSE)
        {
			...
			else
			{
                $data['breadcrumbs'] 	= $this->menus->create_breadcrumb(
																	  ...
																	  redirect('/backend/eventos/listado');
            }
			...
			//Informacion del idioma selecionado
			$data['idioma_info'] = $this->load->view('back/ficha/idioma_info', $data, true);
			...
			//Crear Idioma
			if(strtolower($accion) == 'normal') //Este IF es un PQC. Asi mismo!
			$data['idioma_nuevo'] = $this->load->view('back/ficha/idioma_evento', $data, true); //Vista para la creación de nuevos idiomas
	...
	function _validar_url($url)
	{
		...
		$datos = modules::run('services/relations/tabla_url', 'evento', $url);
		$id_detalle_evento = ($this->input->post('id_detalle_evento')) ? $this->input->post('id_detalle_evento') : '';
		
		if(!empty($datos))
		{
			if(!empty($id_detalle_evento) && $id_detalle_evento == $datos[0]->id_detalle_evento && $datos[0]->id_idioma == $this->input->post('id_idioma'))
			{
				return TRUE;
			}
			else return FALSE;
