...
class evento_model extends CI_Model {
	...
	function total_rows()
  {
  	$this->db->select('detalle_evento.*, evento.*, multimedia.fichero');
		...
		$this->db->order_by('detalle_evento.fecha', 'desc');
	...
	function get_events($por_pagina, $segmento) 
  {
  	$this->db->select('detalle_evento.*, evento.*, multimedia.fichero');
		$this->db->join('detalle_evento', 'evento.id_evento = detalle_evento.id_evento');
		$this->db->join('rel_evento_multimedia', 'evento.id_evento = rel_evento_multimedia.id_evento', 'left');
		...
		$query = $this->db->get('evento', $por_pagina, $segmento);
	...
	function get_event_details($url) 
  {
	...
	{
		$this->db->select('evento.*,detalle_evento.*, evento.id_evento as id_evento, detalle_categoria.nombre as nombre_categoria');
		...
		//Categoria
		$this->db->join('categoria','evento.id_categoria=categoria.id_categoria', 'left');
	...
	function get_page($start = 0, $count = 10, $order_field='evento.id_evento', $order_dir = '', $call = 'back',  $terminos_busqueda = array())
	{
		...
		$this->db->select('	evento.*, detalle_evento.id_detalle_evento, detalle_evento.id_idioma, detalle_evento.nombre,
							detalle_evento.url, detalle_evento.id_idioma, detalle_evento.descripcion_breve, detalle_evento.descripcion_ampliada,
							detalle_evento.lugar, detalle_evento.keywords, detalle_evento.titulo_pagina,
							detalle_categoria.nombre as nombre_categoria, detalle_categoria.descripcion_breve as descripcion_categoria,
							group_concat(m1.fichero) as ficheros1,
							group_concat(m2.fichero) as ficheros2, 
							group_concat(m4.fichero) as ficheros4', FALSE);
		...
		if (!empty($terminos_busqueda))
		{
			...
			{
				if( ($field == 'evento.id_producto' || $field == 'evento.destacado' || $field == 'detalle_evento.id_idioma') && $value != '')
				{
	...
	function update($data)
	{
		...
		{
			//Update
			$data['id_usuario'] = ($this->session->userdata('id_usuario')) ? $this->session->userdata('id_usuario') : NULL;
	...
	function get_categorias($ajax = FALSE)
	{
		$this->db->join('detalle_categoria dc', 'dc.id_categoria = c.id_categoria');
		$query = $this->db->get('categoria c')->result();
	...
	function get_id_evento_url($url)
	{
		$this->db->where('dp.url', $url);
		
		$query 		= $this->db->get('detalle_evento dp');
		$resultado 	= $query->result();
		
		if($query->num_rows() > 0)
		{
			return $resultado[0]->id_evento;
		}
		else
		{
			return FALSE;
	...
	function get_eventos_reuniones_deprecado()
	{
		$query = 
		"
			SELECT ee.*, e.*, mu.fichero as fichero
			FROM evento e
			JOIN detalle_evento ee ON ee.id_evento = e.id_evento
			
			LEFT JOIN (	SELECT m.*, r.id_evento
						FROM rel_evento_multimedia r
						JOIN multimedia m ON m.id_multimedia = r.id_multimedia
						WHERE m.destacado = 1) mu ON mu.id_evento = e.id_evento
						
			WHERE e.id_estado = 1
			GROUP BY e.id_evento
			ORDER BY e.destacado ASC, e.creado DESC
		";
		return $this->db->query($query)->result();
	}
	
	function get_eventos_reuniones()
	{
		$query = 
		"
			SELECT ee.*, e.*, group_concat(m.fichero SEPARATOR ',') as fichero
			FROM evento e
			JOIN detalle_evento ee ON ee.id_evento = e.id_evento
			
			LEFT JOIN rel_evento_multimedia r ON r.id_evento = e.id_evento
			LEFT JOIN multimedia m ON m.id_multimedia = r.id_multimedia
	...
	function get_evento_url($url)
	{
		...
		"
			...
			WHERE e.id_estado = 1 AND ee.url = ".$this->db->escape($url)."
			GROUP BY e.id_evento
			ORDER BY e.destacado ASC, e.creado DESC
		";
		return $this->db->query($query)->result();
