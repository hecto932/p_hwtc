<div class="row">
	<div class="twelve columns">
		...
		<table class="twelve" border="1" summary="Tabla de galerias.">
				...
				<?php foreach ($galerias as $galeria): ?>
					<tr>
						...
						<td class="col1" headers="nombre">
							<p>
								<?php echo ($galeria->nombre != '') ? ucfirst($galeria->nombre) : ucfirst(lang('galerias.titulo_sing').' '.lang('sin_titulo'));  ?>
							</p>
						</td>
						<td class="col3" headers="destacado">
							<p>
								<?php echo ((!empty($galeria->destacado)) ? $galeria->destacado : lang('listado_no')); ?>
								<?php //echo (($galeria->destacado == '1') ? lang('listado_si') : lang('listado_no')); ?>
							</p>
						</td>
						...
						<td class="col3" headers="categoria">
							<p>
								<?php echo ucfirst($galeria->nombre_categoria); ?>
							</p>
						</td>
						-->

						<td class="col4" headers="fecha"><p><?php echo date('d/m/Y',mysql_to_unix($galeria->creado))?></p></td>

						<td class="col6" headers="estado">
							<p>
								<?php
									$estado = json_decode(modules::run('services/relations/get_from_id','estado', $galeria->id_estado,'true'));
									echo lang($estado->estado);
								?>
							</p>
						</td>
						
						<td class="col10 last" headers="ver_galeria">
							<?php
								if($this->session->userdata('idioma') == 'es')
									echo anchor(lang('backend_url').'/'.lang('galerias_url').'/'.lang('ficha_url').'_'.lang('galeria_url').'/'.$galeria->id_galeria, lang('listado_ver'),array('title'=> lang('listado_ver'), 'class' => 'button radius wtc'));
								else
									echo anchor(lang('backend_url').'/'.lang('galerias_url').'/'.lang('articulo_url').'_'.lang('ficha_url').'/'.$galeria->id_galeria, lang('listado_ver'),array('title'=> lang('listado_ver') , 'class' => 'button radius wtc'));
							?>
						</td>

						<td class="col9" headers="eliminar">
							<p class="centered">
								<?php echo anchor('backend/borrar_galeria/'.$galeria->id_galeria,'Eliminar',array('title'=>"eliminar galeria", 'class'=>"delete", 'id'=>"icon_eliminar"))?>
							</p>
						</td>
