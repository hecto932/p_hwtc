...
class Backend extends MX_Controller {
	...
	function borrar_subscriptor($id = '', $ajax = false)
	{
		if ($id == '') redirect('backend/promociones/subscriptores');
		$res = modules::run('promocion/subscriptores/delete_subscriptor',$id,$ajax);
		modules::run('services/monitor/add','subscriptor',$id,$this->session->userdata('id_usuario'),'borrar');
		redirect('backend/promociones/subscriptores');
	}
	
	function borrar_restaurante($id='',$ajax=false)
	{
		if ($id=='') redirect('backend/restaurantes');
		$res=modules::run('restaurante/delete',$id,$ajax);
		modules::run('services/monitor/add','restaurantes',$id,$this->session->userdata('id_usuario'),'borrar');
	...
	function borrar_galeria($id='',$ajax=false)
	{
		if ($id=='') redirect('backend/galeria');
		$res=modules::run('galeria/delete',$id,$ajax);
		modules::run('services/monitor/add','galeria',$id,$this->session->userdata('id_usuario'),'borrar');
