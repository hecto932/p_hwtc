...
class Noticia_front extends MX_Controller {
	...
	function blog($order_field = 'noticia.id_noticia', $order_dir = 'desc', $start = 0, $estado_noticia = 0){
		...
		$limit = 3;
		$data['url']	= lang('ultimas_url').'-'.lang('noticias_url').'-'.lang('wtc_url');
		...
		$data['num_noticias'] = count($this->noticia_model->get_page(0, 2000, $order_field, $order_dir, 'front', array('noticia.id_estado' => 1,'detalle_noticia.id_idioma' => $this->id_idioma)));
		...
		//nuevo title
		$data['title'] = lang('noticias.meta.title').' - '.lang('menu.wtc-valencia');
		
		
		$data['noticias_archivo'] = $this->noticia_model->get_page(0, 6, 'noticia.id_noticia', 'desc', 'front', array('noticia.id_estado' => 1,'detalle_noticia.id_idioma' => $this->id_idioma));
