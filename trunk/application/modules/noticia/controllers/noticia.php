...
class Noticia extends MX_Controller
{
    ...
    function listado($order_field = 'noticia.id_noticia', $order_dir = 'desc', $start = 0, $ajax = false, $permisologia = '') {
		...
		if (isset($_POST['destacado'])) {
            $terminos_busqueda['noticia.destacado'] = $_POST['destacado'];
        }
	...
	function _validar_url($url)
	{
		...
		$datos = modules::run('services/relations/tabla_url', 'noticia', $url);
		$id_detalle_noticia = ($this->input->post('id_detalle_noticia')) ? $this->input->post('id_detalle_noticia') : '';
		
		if(!empty($datos))
		{
			if(!empty($id_detalle_noticia) && $id_detalle_noticia == $datos[0]->id_detalle_noticia && $datos[0]->id_idioma == $this->input->post('id_idioma'))
			{
				return TRUE;
			}
			else return FALSE;
