		...
		<dl class="tabs contained four-up">
			<dd <?php echo ($sub_activo == 'Ficha') ? 'class="active"' : '' ; ?>>
				<a href="#Ficha">
					...
					<span><?php echo lang('listado_ficha_del').' '.lang('promocion_url'); ?></span>
				</a>
			</dd>
			<dd>
				<a href="#Imagenes">
			...
			?>
			<dd>
				<a href="#Lang<?php echo $promocion_idioma->id_idioma ?>" title="">
					<i class="foundicon-globe"></i>
					<span> <?php echo ucfirst($idioma[$promocion_idioma->id_idioma]->nombre); ?> </span>


				</a>
			</dd>
			<?php endforeach; ?>
			<?php if (count($idiomas) > count($promocion_idiomas) && ($accion != 'editar')): ?>
