...
class Categoria extends MX_Controller {
	...
	function listado($order_field='id_categoria',$order_dir='desc',$start=0,$ajax=false)
	{
		...
		if ($start==0 && empty($_POST) && $order_field=='id_categoria') $this->session->unset_userdata('terminos_busqueda');
		...
		if ($config['total_rows']==0) redirect('categoria/buscar/ningun_resultado');
		...
		if ($ajax){
			...
			//	if ($lbt=='categoria.id_categoria'){
				...
				$data['breadcrumbs']=array('categoria'=>'Categorias','buscar'=>'Busqueda','titulo'=>$lbc);
			}else{
				$data['breadcrumbs']=array('categoria'=>'Categorias','listado'=>'Listado');
			}
	...
	function buscar($mensaje='')
	{
		$data['active']='categoria';
		$data['sub']='buscar';
		$data['title']='Busqueda de categorias';
		$data['breadcrumbs']=array('categoria'=>'Categorias','buscar'=>'Busqueda de categorias');
	...
	function crear()
	{
		...
		$data_content['arbol_categorias']=modules::run('services/relations/arbol_categorias','0','0');
	...
	function create($id=''){
		...
		{	
			...
			if ($id!=''){
				...
				$data['breadcrumbs']['edit']='Listado';
				$data['breadcrumbs'][$id]=$data_content['categoria']->nombre;					
			}else{
				$data['breadcrumbs']=array('categoria'=>'Categorias','crear'=>'Crear un categoria');
			}
			$data['main_content'] = $this->load->view('crear_categoria',$data_content,true);
		...
		}else
		{
			...
			if (isset($img) && $img!=''){
				...
				modules::run('services/relations/insert_image',$img,$id,'categoria');
	...
	function edit($id='',$ajax=false){
		...
		$data['active']='categoria';
		$data['sub']='editar';
		if ($id=='') redirect('backend/categoria');
		//$data['relations'] = $this->load->view('relations','',true);
		...
		$data['breadcrumbs']=array('categoria'=>'Categorias','backend/editar_categoria'=>'Editar una categoria',$id=>$data_content['categoria']->nombre);
		$data['title']='Editar '.$data_content['categoria']->nombre;
		$data['main_content'] = $this->load->view('crear_categoria',$data_content,true);
	...
	function ficha($id=''){
		...
		$data['active']='categoria';
		$data['sub']='editar';
		$data['title']='Ficha de categoria';
		//$data['relations'] = $this->load->view('relations','',true);
		if(!$data_content['categoria']=$this->categoria_model->read($id)) redirect('backend/categorias');
		...
		$data['breadcrumbs']=array('categoria'=>'Categorias','ficha'=>'Ficha de la categoria',$id=>(isset($data_content['categoria']->nombre) ? 'Ficha de '.$data_content['categoria']->nombre : 'Categoria sin nombre'));
		$data['nombre']=(isset($data_content['categoria']->nombre) ? 'Ficha de '.$data_content['categoria']->nombre : 'Categoria sin nombre');
	...
	function editar_idioma($id_categoria,$id_detalle_categoria=''){
		if ($id_detalle_categoria=='') redirect('backend/ficha_categoria/'.$id_categoria);
...
if ($this->form_validation->run($this) == FALSE)
	...
	*/
	function eliminar_idioma($id,$ajax=false){
		...
		else redirect('backend/ficha_categoria/'.$detalle->id_categoria);
	}
	...
	function categorias_categoria($id_categoria,$ajax=1){
		...
		else return modules::run('services/relations/get_from_categoria',$id_categoria,'categoria',$ajax);
	}
}
